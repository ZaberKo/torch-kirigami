...
def _domains(space, axes=None):
    space.graph.validate()
    axes = space.axes if axes is None else axes
    declarations = {}
    for op in space.graph.operations():
        for domain in space.graph.operator_spec(op).candidates:
...
class CumulativeChannelBudget:
    ...
    budget() is read-only. After an applied plan, call update(result, new_space)
    ...
    def __init__(self, space, *, scope="local"):
        if scope not in ("local", "global"):
            raise ValueError("scope must be local or global")
        self.scope = scope
        self._domains = _domains(space)
        self._initial = tuple(a.tensor.shape[a.dim] for a in space.axes)
        self._current = self._initial
        self._structure = snapshot(space.graph.model, guarded=space.graph.constant_guards())

    def _validate_space(self, space, structure, widths, *, domains=None):
        domains = self._domains if domains is None else domains
        try:
            axes = tuple(space.graph.parameter(paths[0]).axis(dim) for paths, dim, _ in domains)
        ...
        if (
            ...
            or not set(space.axes).issubset(axes)
            or not set(axes).issubset((*space.axes, *space.protected_axes))
            or _domains(space, axes) != domains
        ):
        ...
        check_structure(space.graph.model, structure)
        return axes

    def budget(self, space, ratio):
        ...
        axes = self._validate_space(space, self._structure, self._current)
    ...
    def update(self, result, space):
        ...
        self._validate_space(space, result.structure, expected)
    ...
    def load_state_dict(self, state, space):
        ...
        self._validate_space(space, structure, current, domains=domains)
