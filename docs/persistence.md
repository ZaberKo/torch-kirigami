...
The checkpoint includes parameters, persistent buffers, and explicitly saved nonpersistent registered buffers. It preserves supported tensor layouts, parameter `requires_grad`, tensor sharing by object identity, module sharing, and saved module configuration, including training flags. `map_location` controls storage relocation using PyTorch loading semantics.
