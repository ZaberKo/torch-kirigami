...
| Conditional scoring and dynamic selection | [conditional metrics](../tests/pruning/test_conditional_metrics.py), [dynamic strategy](../tests/pruning/test_dynamic_strategy.py), [context](../tests/pruning/test_context.py) | Joint-only effects, row/column overlap, normalized batch/subset/block invariance, statistic cache lifetime, complete commits and independently verified strategy results |
| Independent-axis score reuse | [independent ranking](../tests/pruning/test_independent_ranking.py) | Static/dynamic plan and restored execution equivalence, independent norms at extreme scales, aliases and repeated calls, noncontiguous weights, unchanged constraint completion, explicit generic fallbacks and propagation-count bounds |
...
| Paper-based workflow criteria | [FPGM criterion](../tests/integration/test_geometric_metric.py), [VBP workflow](../tests/integration/test_variance_workflow.py), [VBP operator extension](../tests/integration/test_variance_operators.py) | Signed filter distances, streaming activation variance, independent mean-replacement references, real ConvNeXt/ViT blocks, guarded eval capture, calibration cleanup and corrected checkpoint restoration on CPU/CUDA |
| Reconstruction search | [OSSCAR workflow](../tests/integration/test_osscar_workflow.py) | Independent least-squares fits, grouped Schur updates, improving swaps, teacher/current calibration, sequential target allocation and refitted checkpoint restoration |
| Independent structural-family selection | [Isomorphic workflow](../tests/integration/test_isomorphic_workflow.py) | Dependency signatures, equivalent-closure deduplication, independent family quotas, retained-width rounding and public plan/apply/checkpoint lifecycle without Greedy |
| ViT whole-head and FFN selection | [Head-pruning workflow](../tests/integration/test_vit_head_workflow.py) | Native-attention conversion, independent dense head-mask outputs and gradients, static score formulas, joint compaction, nonempty/partial-head rejection, training and checkpoint restoration on CPU/CUDA; CLI-to-import restoration in separate processes |
