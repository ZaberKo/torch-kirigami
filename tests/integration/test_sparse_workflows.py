...
def test_residual_grouped_cnn_sparse_training_and_independent_compact_reference(execution_device):
    ...
    plan = pruner.plan(remove=(space.graph.parameter("stem.weight").axis(0).select([0, 4]),))
    pruner.apply(plan)
    torch.testing.assert_close(model(x), reference)
    with pytest.raises(StaleGraphError):
        regularizer()
    fresh = build_space(model, x)
    optimizer = torch.optim.AdamW(model.parameters(), lr=0.01)
    optimizer.zero_grad()
    (model(x).sum() + 0.01 * GroupLasso(fresh.parameter_groups())()).backward()
    optimizer.step()
...
def test_transformer_head_gate_compaction_and_checkpoint(execution_device):
    ...
    binding = GateBinding(space.graph, "attn.gate")
    counts = tuple(4 if a.tensor.paths[0] == "attn.k.weight" else 0 for a in space.axes)
    plan = Pruner(model, graph=space.graph).plan(
        metric=GateMagnitude((binding,)),
        candidates=binding.candidates(space),
        budget=ChannelCount(counts, space.axes),
    )
    Pruner(model, graph=space.graph).apply(plan)
...
def test_example_training_checkpoint_restores_optimizer_rng_and_algorithm(tmp_path):
    ...
    x = torch.randn(3, 8)
    space = build_space(model, x)
    Pruner(model, graph=space.graph).apply(
        Pruner(model, graph=space.graph).plan(
            remove=(space.graph.parameter("hidden.weight").axis(0).select([1, 4]),),
        )
    )
    optimizer = torch.optim.AdamW(model.parameters(), lr=0.01)
...
def test_soft_example_regrowth_keeps_momentum_and_can_compact(operation, execution_device):
    ...
    pruner = Pruner(model, graph=space.graph)
    plan = pruner.plan(metric=Magnitude(), budget=ChannelRatio(0.5))
    selected = [c for c in space.candidates if c.key in plan.selected]
    group = ParameterGroup(space.graph, space.impact(selected).parameters)
    ...
    train_steps(model, x, y, optimizer, 1)
    assert GroupLasso((group,))().item() > 0
    pruner.apply(pruner.plan(metric=Magnitude(), budget=ChannelRatio(0.5)))
    build_space(model, x)
    optimizer = torch.optim.SGD(model.parameters(), lr=0.01)
    train_steps(model, x, y, optimizer, 1)
    stream = io.BytesIO()
