...
def test_l2_extreme_finite_values_preserve_scores_and_order(scale, dtype, execution_device):
    ...
    context = PlanningContext(
        graph, graph.operations(), candidates, ChannelRatio(0.5), (axis,), Magnitude(), ()
    )
    expected = [math.hypot(*row) for row in model.weight.detach().cpu().tolist()]
    assert context.score(candidates) == pytest.approx(expected, rel=1e-6, abs=0)
    plan = Pruner(model, graph=graph).plan(
        metric=Magnitude(), budget=ChannelRatio(0.5), preserve_io=False
    )
...
def test_stable_l2_complex_components_and_zero_region(execution_device):
    ...
    assert context.score(candidates) == pytest.approx((math.hypot(value.real, value.imag), 0))
...
def test_taylor_promotes_before_multiplying(scale, mode, execution_device):
    ...
    context = PlanningContext(
        graph, graph.operations(), candidates, ChannelRatio(0.5), (axis,), metric, ()
    )
    ...
    assert context.score(candidates) == pytest.approx(expected, rel=1e-12, abs=0)
    plan = Pruner(model, graph=graph).plan(
        metric=metric, budget=ChannelRatio(0.5), preserve_io=False
    )
...
def test_complex_l1_promotes_before_absolute_value(execution_device):
    ...
    candidates = tuple(Candidate(str(i), (axis.select([i]),)) for i in range(2))
    context = PlanningContext(
        graph, graph.operations(), candidates, ChannelRatio(0.5), (axis,), Magnitude(1), ()
    )
    expected = [
        math.fsum(math.hypot(v.real, v.imag) for v in row)
        for row in model.weight.detach().cpu().tolist()
    ]
    assert context.score(candidates) == pytest.approx(expected, rel=1e-12)
